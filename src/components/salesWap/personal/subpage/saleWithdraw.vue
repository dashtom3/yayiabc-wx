  ...
  <div class="saleWithdraw-container">
    ...
    <div class="withdraw-pending" v-if="!withdrawSuccess">
      ...
      <div class="sale-confirm-container">
        ...
        <mt-field label="验证码" placeholder="请输入短信验证码" v-model="submitWitParams.vCode" :attr="{ maxlength: 4 }">
          <mu-raised-button :label="captchaBtnText"
                            ...
                            :backgroundColor="(captchaBtnText==='获取验证码' || captchaBtnText === '再次发送')?'rgb(54, 118, 182)':'#e6e6e6'"
                            class="btn"
                            @click="getCaptcha"/>
        </mt-field>
      </div>
      <div class="withdraw-money-container">
        ...
        <input type="text" placeholder="请输入提现金额" @change="testWithdrawMoney"
               v-model="submitWitParams.moneyNnm" class="input-money"/>
        <p class="withdraw-conunt primary">可提现余额 ￥ {{saleInfo.money}}</p>
      </div>
      <div class="withdraw-btn-container">
        <mu-raised-button label="申请提现" fullWidth color="#fff" backgroundColor="#3676b6" class="logout-btn"
    ...
    methods: {
      ...
      testWithdrawMoney(e) {
        if (e.target.value > this.saleInfo.money) {
          Toast('您输入的余额超过上限')
          this.submitWitParams.moneyNnm = this.saleInfo.money
        }
      },
      goPage(name) {
